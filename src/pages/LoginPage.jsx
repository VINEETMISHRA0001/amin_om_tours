...
const LoginPage = () => {
  ...
  const { error, loading, token } = useSelector((state) => state.auth); // Add token here to check if login is successful
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');

  // Track changes in auth state to handle navigation
  useEffect(() => {
    if (token) {
      navigate('/'); // Redirect to dashboard or home after successful login
    }
  }, [token, navigate]); // Navigate when the token is set
